...
本项目的目的是通过不断向RAM Disk中覆写数据以积攒使用者的赛博功德。

经过不严谨的计算，每点赞一次地狱笑话需要抄写一部经书，主动讲一次需要抄写一百部，请各位罪人自觉抄写，以免功德为负时掉入赛博地狱。

## 数据介绍

由于不太清楚抄什么经书好，索性把已知经书全抄一遍吧。本项目使用的佛经来自[CBeta项目](https://cbeta.org/ebooks)：``https://cbeta.org/ebooks``，佛经部分内容按照**CC BY-NC-SA 3.0 TW**协议授权使用，援引项目资料，共包括如下佛经：
...
共包括21956个文件（文件列表见``ret/index.csv``），几乎包括了当前已知的所有佛经，其统计数据如下：
...
使用``analyze_sutra.py``统计排名前1000的词语的词频，文件保存在``ret/word_freq.csv``，前20名如下。这个表的后面更有意思，建议下载后查阅。
...
uv初始化环境，创建一个RAM Disk，修改一下config.json，执行main.py开跑（后续补充详细的）
...
这个速度还是受限了，比理论速度低挺多，我这内存理论上限是
$$
3600 \times 8 \times 2 = 57,600 \text{ MB/s} \approx \mathbf{57.6 \text{ GB/s}}
$$
这才1/3，真实速度或许还得在Linux上开tmpfs跑着看看。以后有时间再吧（

其实文件在162m这个大小，低于Cache的斩杀线了，如果有服务器级CPU，应该能在L3 Cache里开缓存盘，那速度更是成百上千倍的提升。
